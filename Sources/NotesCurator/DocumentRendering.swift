...
struct StyledEditorDocumentPreview: View {
    let version: DraftVersion
    ...
    private var border: Color {
        Color(hex: theme.borderHex) ?? accent.opacity(0.16)
    }

    private var renderedDocument: AttributedString {
        let source = version.editorDocument.trimmingCharacters(in: .whitespacesAndNewlines)
        let fallback = source.isEmpty
            ? (version.outputLanguage == .chinese ? "开始编辑后，这里会显示最新预览。" : "Start editing and the latest preview will appear here.")
            : source
        return (try? AttributedString(markdown: fallback)) ?? AttributedString(fallback)
    }
